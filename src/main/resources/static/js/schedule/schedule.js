...
document.addEventListener('DOMContentLoaded', function() {
	...
	const calendar = new FullCalendar.Calendar(calendarEl, {
		...
		eventClick: function(e) { // 일정 클릭 시 일정 상세보기
			...
			if (!$(e.el).hasClass("koHolidays")) {
				...
				$(".calendarModal__calName").text(datas.extendedProps.calName);
				...
				$(".calendarModal__delete").on("click", () => { calendar.getEventById($("#eventId").val()).remove(); $.modal.close(); }) // 일정 삭제
			} else {
	...
	function generateEvent() {
		...
		let eventData = {
			...
			reg_date: new Date().toISOString().slice(0, 16), // 나중에 ajax로 insert 쿼리 날린 후 값 가져오기.. (now() 해서 날짜값...)  
			...
			if (during == 1) { // 1일짜리 이벤트 일 때
				...
				if (endKey == "count") { // 반복 횟수 지정
					for (let i = 0; i < endValue; i++) {
						setEvents(events, endDate, eventData, currentDate, during, frequency__whenOption, term);
					}
				} else { // 종료 일자 지정
	...
	// 이벤트 생성
	let setEvents = (events, endDate, eventData, currentDate, during, frequency__whenOption, term) => {
		let ed = { ...eventData };
