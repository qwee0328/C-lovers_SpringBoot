$(document).ready(function() {
	...
	function pagination(recordTotalCount, recordCountPerPage, naviCountPerPage, lastPageNum) {
		...
		if(recordTotalCount != 0) {
			...
			if (startNavi != 1) {
				let divTag = $("<div>");
				divTag.attr("href", "/mail/tempBoxList?cpage=1");
				let iTag = $("<i>");
			...
			if (needPrev) {
				let divTag = $("<div>");
				divTag.attr("href", "/mail/tempBoxList?cpage=" + (startNavi - 1));
				let iTag = $("<i>");
			...
			for (let i = startNavi; i <= endNavi; i++) {
				let divTag = $("<div>");
				divTag.text(i);
				divTag.attr("href", "/mail/tempBoxList?cpage=" + i);
				if (i == currentPage) {
			...
			if (needNext) {
				let divTag = $("<div>");
				divTag.attr("href", "/mail/tempBoxList?cpage=" + (endNavi + 1));
				let iTag = $("<i>");
