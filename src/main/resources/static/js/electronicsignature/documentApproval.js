$(document).ready(function() {
	// 결재 리스트 출력
	$.ajax({
		url: "/electronicsignature/documentApprovalList?cpage="
	}).done(function(resp){
	...
	function pagination(recordTotalCount, recordCountPerPage, naviCountPerPage, lastPageNum) {
		...
		if(recordTotalCount != 0) {
			...
			if (startNavi != 1) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentApprovalList?cpage=1");
				let iTag = $("<i>");
			...
			if (needPrev) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentApprovalList?cpage=" + (startNavi - 1));
				let iTag = $("<i>");
			...
			for (let i = startNavi; i <= endNavi; i++) {
				...
				divTag.text(i);
				divTag.attr("href", "/electronicsignature/documentApprovalList?cpage=" + i);
				if (i == currentPage) {
			...
			if (needNext) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentApprovalList?cpage=" + (endNavi + 1));
				let iTag = $("<i>");
			...
			if (endNavi != pageTotalCount) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentApprovalList?cpage="+pageTotalCount);
				let iTag = $("<i>");
