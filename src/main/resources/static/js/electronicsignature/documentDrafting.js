$(document).ready(function() {
	// 기안 리스트 출력
	$.ajax({
		url: "/electronicsignature/documentDraftingList?cpage="
	}).done(function(resp){
	...
	function pagination(recordTotalCount, recordCountPerPage, naviCountPerPage, lastPageNum) {
		...
		if(recordTotalCount != 0) {
			...
			if (startNavi != 1) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentDraftingList?cpage=1");
				let iTag = $("<i>");
			...
			if (needPrev) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentDraftingList?cpage=" + (startNavi - 1));
				let iTag = $("<i>");
			...
			for (let i = startNavi; i <= endNavi; i++) {
				...
				divTag.text(i);
				divTag.attr("href", "/electronicsignature/documentDraftingList?cpage=" + i);
				if (i == currentPage) {
			...
			if (needNext) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentDraftingList?cpage=" + (endNavi + 1));
				let iTag = $("<i>");
			...
			if (endNavi != pageTotalCount) {
				let divTag = $("<div>");
				divTag.attr("href", "/electronicsignature/documentDraftingList?cpage="+pageTotalCount);
				let iTag = $("<i>");
